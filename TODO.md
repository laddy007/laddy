...
- laddy-as-target dogfood: gate image now carries the scanner set (pytest-cov/diff-cover/semgrep/gitleaks, pinned in `.laddy/docker/Dockerfile.test`). REMAINING: `.laddy/policy.toml` still holds the generic template (`coverage_package = "myapp"` + `myapp/*` globs) - set `coverage_package = "orchestrator"` and laddy-real sensitive/security globs, else the binding gate's coverage step fails on changed `orchestrator/` lines.
